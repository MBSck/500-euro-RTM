...
void hspiLoop(void* unused){
    ...
    while(1){
        ...
        if ((configExisting == 0x3FF) && !dataReady){ //0x3FF is 0b11111111111 -> ten ones
            //handshake line high
            gpio_set_level(GPIO_HANDSHAKE_HSPI, 1);
            configNeeded = false;
...
void vspiLoop(void* unused){
    ...
    while(1) {
        ...
        if(currentXDac != lastXDac){ //only if new value has been written to currentXDac
            vspiSendDac(currentXDac, buffer.get(), handleDacX); // Dac X
            lastXDac = currentXDac;
            printf("new X \n");
        }
        if(currentYDac != lastYDac){ //only if new value has been written to currentYDac
            vspiSendDac(currentYDac, buffer.get(), handleDacY); // Dac Y
        ...
        //vspiSendDac(15000, buffer.get(), handleDacZ); // Dac Z
        //vspiSendDac(15000, buffer.get(), handleDacY); // Dac Y
        //vspiSendDac(15000, buffer.get(), handleDacX); // Dac X
        printf("Z: %d\n", currentZDac);
        vTaskSuspend(NULL); //will be resumed by controller
    }
}
...
esp_err_t spiSend(spi_host_device_t host, spi_slave_transaction_t *current, TickType_t ticks_to_wait, spi_slave_transaction_t *previous){
    ...
    if(previousConfigBad){
        ...
        for(uint16_t attemptCounter = 0; attemptCounter <= maxNumberAttemptsSPI; attemptCounter++){
            ...
            if(!previousReceivedCommand->getConfigBad()){ //if previous transaction has been transmitted succesfully
                break;
    ...
    if(currentConfigBad){
        ...
        for(uint16_t attemptCounter = 0; attemptCounter <= maxNumberAttemptsSPI; attemptCounter++){
            ...
            if(!currentReceivedCommand->getConfigBad()){ //if previous transaction has been transmitted succesfully
                break;
            }else if(attemptCounter == maxNumberAttemptsSPI){
                printf("ERROR, cant transmit succesfully \n");
                return retHspi;
